...
import { Card, EmptyState, MetricCard, PageHeader, StatusBadge } from "@/components/ui/surface";
...
export default function ComplianceReportPage() {
  ...
  return <DashboardGate><div className="space-y-8 print:space-y-6">
    ...
    {report && <>
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6" aria-label="Resumen de cumplimiento">
        <MetricCard label="Total" value={report.summary.total_obligations} />
        <MetricCard label="Cumplidas" value={report.summary.compliant} tone="success" />
        <MetricCard label="En proceso" value={report.summary.in_progress} tone="warning" />
        <MetricCard label="Pendientes" value={report.summary.pending} />
        <MetricCard label="Vencidas" value={report.summary.overdue} tone="danger" />
        <MetricCard label="Cumplimiento" value={`${report.summary.compliance_percentage}%`} tone="info" />
      </section>

...
export function KpiStrip({ metrics, featuredStyle = "navy", uniformHeight = false }: { metrics: KpiMetric[]; featuredStyle?: "navy" | "light"; uniformHeight?: boolean }) {
  ...
  return <section className={`grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 ${columns}`} aria-label="Resumen de cumplimiento">{metrics.map((metric) => {
    ...
    return <article key={metric.label} className={`group rounded-2xl border p-5 transition duration-200 ease-out motion-safe:hover:-translate-y-px focus-within:ring-2 focus-within:ring-slate-900/10 ${uniformHeight ? "sm:min-h-[154px]" : ""} ${metric.featured ? featuredSurface : `${surface[tone]} border-[var(--border-subtle)] shadow-[inset_0_1px_0_rgba(255,255,255,.68),var(--shadow-sm)] ${hover[tone]}`}`}><div className="flex items-start justify-between gap-3"><div><p className={`text-sm font-medium ${metric.featured ? featuredMutedText : "text-slate-500"}`}>{metric.label}</p><p className={`mt-2 text-3xl font-semibold tracking-tight ${metric.featured ? featuredText : color[tone]}`}>{metric.value}</p></div>{Icon && <span className={`grid h-9 w-9 place-items-center rounded-[10px] transition duration-200 ${metric.featured ? featuredIcon : `bg-white/80 ${color[tone]} shadow-[inset_0_1px_0_rgba(255,255,255,.75),0_1px_2px_rgba(15,23,42,.05)] group-hover:bg-white`}`}><Icon className="h-[18px] w-[18px]" /></span>}</div>{metric.progress !== undefined && <div className={`mt-5 h-1.5 overflow-hidden rounded-full ${metric.featured && !isLightFeatured ? "bg-white/15" : "bg-slate-200/80"}`}><div className="h-full rounded-full bg-emerald-400" style={{ width: `${metric.progress}%` }} /></div>}{metric.helper && <p className={`mt-3 text-xs ${metric.featured ? featuredMutedText : "text-slate-500"}`}>{metric.helper}</p>}</article>;
  })}</section>;
}

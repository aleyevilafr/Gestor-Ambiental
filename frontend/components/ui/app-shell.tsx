...
export function AuthenticatedHeader({ user }: { user: AuthenticatedUser }) {
  return <header className="no-print sticky top-0 z-20 border-b border-white/10 bg-[#111c30] text-white"><div className="mx-auto flex min-h-14 w-full max-w-[1600px] items-center gap-5 px-4 py-2 md:px-6 lg:gap-8 lg:px-8 xl:px-10"><div className="shrink-0"><BrandMark tone="dark" compact title="Cumplimiento Ambiental" /></div><div className="min-w-0 flex-1 lg:flex lg:justify-center"><AppNavigation user={user} /></div><div className="hidden min-w-0 items-center gap-2 rounded-md border border-white/10 bg-white/5 px-3 py-2 text-right text-[15px] transition hover:bg-white/10 sm:flex"><BuildingIcon className="h-4 w-4 shrink-0 text-slate-300" /><div className="min-w-0"><p className="truncate font-medium">{user.organization.name}</p><p className="truncate text-[13px] text-slate-300">{user.name} · {user.role === "ADMIN" ? "Administrador" : user.role === "RESPONSIBLE" ? "Responsable" : "Lector"}</p></div><ChevronDownIcon className="h-4 w-4 shrink-0 text-slate-400" /></div></div></header>;
}

...
export type AIObligationProposal={title:string;description:string|null;matter:string|null;regulatory_source:string|null;article:string|null;deadline:string|null;frequency:string|null;confidence:number|null;source_excerpt:string|null};
export type AIObligationAnalysisResponse={document_name:string;proposals:AIObligationProposal[];warnings:string[]};
export async function analyzeObligations(file:File){const response=await fetch(`${apiUrl}/api/v1/ai/analyze-obligations`,{method:"POST",credentials:"include",headers:{"Content-Type":file.type||(/\.pdf$/i.test(file.name)?"application/pdf":"text/plain"),"X-Document-Name":file.name},body:file});if(!response.ok){const body=await response.json().catch(()=>({}));throw new ApiError(typeof body.detail==="string"?body.detail:"No fue posible analizar el documento.",response.status)}return response.json() as Promise<AIObligationAnalysisResponse>}
